...
void TrackAnalysis::Loop(Long64_t maxentries)
{
    ...
    const unsigned short int NETABIN = 4, // 4 eta bins of width 0.6 
                             NKIN = 7, // KINematics, plus multiplicity
                             NDIR = 4; // DIRections
    const double ETABINWIDTH = maxtracketa/((double) NETABIN);

    unsigned int  totaltracks = 0; // #tracks passing the cutoffs
    /************************* BOOKING HISTOGRAMS *****************************/
    ...
    const TH1D * proto_tracks[NKIN] = { new const TH1D ("pt" , "transverse momentum ;p_T/GeV;#entries",   500,     0,   50),
                                        new const TH1D ("eta", "pseudorapidity ;#eta   ;#entries",   50,  -2.5,  2.5),
                                        new const TH1D ("phi", "azimuthal angle ;#phi   ;#entries",   20,   -PI,   PI),
                                        new const TH1D ("M"  , "multiplicity ;M      ;#enitres",  1500,     0,  1500),
                                        new const TH1D ("dxy_errxy","d_{xy}/#sigma_{xy};d_{xy}/#sigma_{xy};Fraction of tracks",50,0.0,10.0),
                                        new const TH1D ("pt_errpt","sigma_{pt}/p_{t};#sigma_{pt}/p_{t};Fraction of tracks",100,0.0,0.1),
                                        new const TH1D ("dz_errz","d_{z}/#sigma_{z};d_{z}/#sigma_{z};Fraction of tracks",50,0.0,10.0)},
               * proto_vertices[NDIR]   = { new const TH1D ("x", "x-position;x/mm;#entries", 100,  -1.0, 1.0),
                                            ...
                                            new const TH1D ("Nvertices","Number-good-vertices;Number of vertices;Fraction of entries",50,0.0,50.0)};
...
#undef DEFINE2DHIST
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++)
    {
        ...
        //Leading energetic HF tower(RT) and HF particle(GT)
        //double leading_GT_E = 0,
        //       leading_RT_E = 0;
        ...
        if (kContainsGenLevel)
        {
            ...
            hist1D["GV_Nvertices"]->Fill(RV.x->size());
        ...
        for (unsigned int itrack = 0 ; itrack < RT.pt->size() ; itrack++)
        {
                ...
                && nrectracks>0)
            {
                ...
                hist1D["RT_dz_errz"]->Fill((RT.dz->at(itrack))/(RT.dzError->at(itrack))); 
        ...
        //if(nrectracks>0) hist1D["RT_M"]->Fill(nrectracks);
        hist1D["RT_M"]->Fill(nrectracks);
        for (unsigned short int irechit = 0 ; irechit < RH_HF.energy->size() ; irechit++) // TODO: check if a loop is really necessary
    ...
    //Profiles
    //profile["RT_dxy_errxy_VS_eta"]->Scale(1.0/((double)totaltracks));
    profile["RT_dxy_errxy_VS_eta"]->Scale(1.0/0.1);     //BinWidth of X axis of this TProfile
